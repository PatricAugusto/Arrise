...
export default function RegisterScreen() {
  ...
  return <SafeAreaView className="flex-1 bg-bg px-6"><View className="flex-1 justify-center"><View className="mb-9"><Text className="mb-3 font-mono text-[10px] tracking-[2px] text-aurora-600">ARRISE://REGISTER</Text><Text className="font-display text-4xl leading-tight text-text">Crie seu espaço</Text><Text className="mt-3 font-body text-base text-text-dim">Sua rotina, seus sinais, sua conta.</Text></View>
    ...
    <Text className="mt-7 text-center font-body text-sm text-text-dim">Já possui uma conta?</Text><Link href={"/(auth)/login" as any} asChild><Pressable className="mt-2"><Text className="text-center font-body-semibold text-sm text-aurora-600">Entrar</Text></Pressable></Link>
  </View></SafeAreaView>;
}
function Field({ label, ...props }: { label: string } & React.ComponentProps<typeof TextInput>) { return <View className="mb-4"><Text className="mb-2 font-mono text-[10px] tracking-[1.5px] text-text-dim">{label}</Text><TextInput {...props} accessibilityLabel={label} placeholderTextColor="#52606D" className="rounded-2xl border border-glass-border/20 bg-bg-elevated px-4 py-4 font-body text-base text-text" /></View>; }
