...
export default function LoginScreen() {
  ...
  return <AuthShell eyebrow="ARRISE://ACCESS" title="Volte ao seu ritmo" subtitle="Entre para continuar sua sequência.">
    ...
    <Link href={"/(auth)/forgot-password" as any} asChild><Pressable className="mb-6 self-end"><Text className="font-body-medium text-sm text-aurora-600">Esqueci minha senha</Text></Pressable></Link>
    ...
    <Link href={"/(auth)/register" as any} asChild><Pressable className="mt-2"><Text className="text-center font-body-semibold text-sm text-aurora-600">Criar uma conta</Text></Pressable></Link>
  </AuthShell>;
}
...
function AuthShell({ eyebrow, title, subtitle, children }: { eyebrow: string; title: string; subtitle: string; children: React.ReactNode }) {
  return <SafeAreaView className="flex-1 bg-bg px-6"><View className="absolute -right-24 top-12 h-64 w-64 rounded-full border border-aurora-500/10" />
    <View className="flex-1 justify-center"><View className="mb-10"><View className="mb-3 flex-row items-center"><View className="mr-2 h-2 w-2 rounded-full bg-aurora-500" /><Text className="font-mono text-[10px] tracking-[2px] text-aurora-600">{eyebrow}</Text></View><Text className="font-display text-4xl leading-tight text-text">{title}</Text><Text className="mt-3 font-body text-base text-text-dim">{subtitle}</Text></View><View>{children}</View></View>
  </SafeAreaView>;
}

function Field({ label, ...props }: { label: string } & React.ComponentProps<typeof TextInput>) {
  return <View className="mb-4"><Text className="mb-2 font-mono text-[10px] tracking-[1.5px] text-text-dim">{label}</Text><TextInput {...props} accessibilityLabel={label} placeholderTextColor="#52606D" className="rounded-2xl border border-glass-border/20 bg-bg-elevated px-4 py-4 font-body text-base text-text" /></View>;
}
